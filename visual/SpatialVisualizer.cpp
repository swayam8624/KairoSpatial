  ...
  -> KairoGeometry
    ...
    out << R"(        </section>
        <aside class="inspector" aria-live="polite">
            ...
            <span data-inspector-text>Hover, tap, or focus a spatial entity to inspect it.</span>
        </aside>
    </main>
    <script>
        (() => {
            ...
            function describeSpatialElement(element) {
                const card = element.closest(".card");
                const panel = card ? card.querySelector("h2")?.textContent : "Spatial panel";
                const kind = element.dataset.kind || "shape";
                const id = element.dataset.id ? ` id=${element.dataset.id}` : "";
                const layer = element.dataset.layer ? ` layer=${element.dataset.layer}` : "";
                return `${panel}: ${kind}${id}${layer}`;
            }

            function selectSpatialElement(element) {
                if (!element || !element.dataset.kind) {
                    return;
                }
            ...
            document.addEventListener("pointerover", (event) => {
                const spatialElement = event.target.closest("[data-kind]");
                if (spatialElement) {
                    selectSpatialElement(spatialElement);
                }
            });
